...
struct Args {
    ...
    dp::HashAlgorithm algo = dp::HashAlgorithm::SHA3;
};
...
uintmax_t parse_size(const std::string &size_str) {
    ...
    if (std::regex_match(size_str, match, re)) {
        ...
        if (match[2].matched) {
            ...
            else if (unit == 'g') value *= 1024ULL * 1024ULL * 1024ULL;
        }
        return value;
    }
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-h" || arg == "--help") {
            ...
            if (i + 1 < argc) {
                ...
                if (algo_str == "sha256") {
                    args.algo = dp::HashAlgorithm::SHA256;
                } else if (algo_str == "sha3") {
                    args.algo = dp::HashAlgorithm::SHA3;
                } else {
                    std::cerr << "Error: Unknown algorithm '" << algo_str << "'. Available: sha256, sha3" << std::endl;
                    exit(1);
                }
            } else {
                std::cerr << "Error: Missing argument for " << arg << std::endl;
...
void show_help(const char *exec_name) {
    std::cout << "DirPacker v" << VERSION << " (No External Dependencies)\n"
              << "Packs a directory structure into a single text stream.\n\n"
              ...
              << "Example:\n"
              << "  " << exec_name << " . --algorithm sha3 -l 100k --stats -o out.pack\n"
              << std::endl;
}
...
int main(int argc, char **argv) {
    ...
    if (args.show_version) {
        std::cout << "DirPacker v" << VERSION << " (SHA256/SHA3 + Custom Base64)" << std::endl;
        return 0;
    }
    if (args.show_help) {
        show_help(argv[0]);
